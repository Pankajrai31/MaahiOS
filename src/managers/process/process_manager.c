/*
 * Process Manager - Simple and Clean
...
/**
 * Create a process
 * Simple: allocate PCB, add to scheduler queue, return
 */
int process_create(uint32_t entry_point) {
    serial_print("[PROCESS_CREATE] Entry=0x");
    serial_hex32(entry_point);
    serial_print("\n");
    
    /* Check limits */
    if (next_pid >= MAX_PROCESSES) {
        serial_print("[PROCESS_CREATE] ERROR: Max processes\n");
        return -1;
    }
    ...
    /* Initialize PCB - simple and clean */
    pcb->pid = next_pid++;
    ...
    *(--stack) = entry_point;            /* EIP (entry point) */
    ...
    /* PUSHA frame (pushed by irq stub) */
    *(--stack) = 0;  /* EDI */
    *(--stack) = 0;  /* ESI */
    *(--stack) = 0;  /* EBP */
    ...
    *(--stack) = 0;  /* EAX */
    ...
    serial_print("[PROCESS_CREATE] Built initial frame, ESP=0x");
