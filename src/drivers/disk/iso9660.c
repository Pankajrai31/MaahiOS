...
int iso9660_init(void) {
    serial_print("[ISO9660] Initializing...\n");
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (drive && drive->type == ATA_DEVICE_TYPE_ATAPI) {
            g_cdrom_drive_id = i;
            serial_print("[ISO9660] Found ATAPI drive at index ");
            // Print drive ID
            serial_print(i == 0 ? "0" : i == 1 ? "1" : i == 2 ? "2" : "3");
            serial_print("\n");
            break;
        }
    }
    ...
    if (g_cdrom_drive_id == 0xFF) {
        serial_print("[ISO9660] No ATAPI drive found!\n");
        return -1;
    }
    ...
    if (ret != 0) {
        serial_print("[ISO9660] Failed to read volume descriptor\n");
        return -2;
    }
    ...
    if (pvd->identifier[0] != 'C' || pvd->identifier[1] != 'D' ||
        ...
        pvd->identifier[4] != '1') {
        serial_print("[ISO9660] Invalid volume identifier\n");
        return -3;
    }
    
    if (pvd->type != ISO_VD_PRIMARY) {
        serial_print("[ISO9660] Not a primary volume descriptor\n");
        return -4;
    }
    ...
    serial_print("[ISO9660] Root directory at LBA ");
    // Simple hex print for LBA
    serial_print("0x");
    char hex[9];
    for (int i = 7; i >= 0; i--) {
        int nibble = (g_root_dir_lba >> (i * 4)) & 0xF;
    ...
    serial_print("[ISO9660] Initialization complete\n");
    return 0;
}
...
int iso9660_list_boot(iso_file_entry_t *entries, int max_entries) {
    ...
    if (iso9660_find_directory("BOOT", &boot_lba, &boot_size) != 0) {
        serial_print("[ISO9660] /boot directory not found\n");
        return -1;
    }
    
    serial_print("[ISO9660] Found /boot at LBA ");
    char hex[9];
    for (int i = 7; i >= 0; i--) {
        int nibble = (boot_lba >> (i * 4)) & 0xF;
        hex[7-i] = nibble < 10 ? '0' + nibble : 'A' + nibble - 10;
    }
    hex[8] = 0;
    serial_print(hex);
...
int iso9660_read_file(uint32_t file_lba, uint32_t file_size, void *buffer, uint32_t max_size) {
    if (g_cdrom_drive_id == 0xFF) {
        serial_print("[ISO9660] ERROR: Not initialized\n");
        return -1;  // Not initialized
    }
    
    if (!buffer) {
        serial_print("[ISO9660] ERROR: NULL buffer\n");
        return -2;
    }
    ...
    serial_print("[ISO9660] Reading file: LBA=0x");
    char hex[9];
    for (int i = 7; i >= 0; i--) {
        int nibble = (file_lba >> (i * 4)) & 0xF;
        hex[7-i] = nibble < 10 ? '0' + nibble : 'A' + nibble - 10;
    }
    hex[8] = 0;
    serial_print(hex);
    serial_print(" size=");
    for (int i = 7; i >= 0; i--) {
        int nibble = (bytes_to_read >> (i * 4)) & 0xF;
        hex[7-i] = nibble < 10 ? '0' + nibble : 'A' + nibble - 10;
    }
    serial_print(hex);
    serial_print("\n");
    ...
    while (bytes_read < bytes_to_read) {
        ...
        int ret = atapi_read_sector(g_cdrom_drive_id, current_lba, g_iso_sector_buffer);
        if (ret != 0) {
            serial_print("[ISO9660] ERROR: Read sector failed\n");
            return -3;
        }
    ...
    serial_print("[ISO9660] Read complete: ");
    for (int i = 7; i >= 0; i--) {
        int nibble = (bytes_read >> (i * 4)) & 0xF;
        hex[7-i] = nibble < 10 ? '0' + nibble : 'A' + nibble - 10;
    }
    serial_print(hex);
    serial_print(" bytes\n");
...
int iso9660_find_and_read_file(uint32_t dir_lba, uint32_t dir_size, 
    ...
    serial_print("[ISO9660] Searching for file: ");
    ...
    serial_print("[ISO9660] File not found\n");
